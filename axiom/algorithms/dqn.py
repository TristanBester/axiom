...
def main():
    ...
    # Reset the environment with shapes (devices * update_batch_size * num_envs, features)
    key = jax.random.key(0)
    key, *env_keys = jax.random.split(key, n_devices * update_batch_size * num_envs + 1)
    env_states, timesteps = jax.vmap(train_env.reset, in_axes=(0))(
        jnp.stack(env_keys),
    )

    # Reshape the environment states and timesteps to be
    # (devices, update batch size, num_envs, features)
    def reshape_states(x):
        print(
            f"reshaping: {x.shape} -> {(n_devices, update_batch_size, num_envs) + x.shape[1:]}"
        )
        return x.reshape((n_devices, update_batch_size, num_envs) + x.shape[1:])
    ...
    # subkeys = jax.random.split(subkey, 1024)
    # state, timestep = jax.pmap(jax.vmap(train_env.reset), axis_name="device")(
    #     key=subkeys
    # )
